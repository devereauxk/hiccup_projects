...
Double_t linbins(double xmin, double xmax, int nbins)
{
  Double_t bins[nbins+1];
  binwidth = (xmax - xmin) / nbins;
  for (int i = 0; i < nbins+1; i++)
  {
    bins[i] = xmin + binwidth * i;
  }
  return bins;
}

Double_t logbins(double xmin, double xmax, int nbins)
{
  Double_t bins[nbins+1];
...
void RooSimple3Dpp_EEC(TString mc_file="preprocessed_mc.root", mc_data_ std::string tag = "", std::string date = "", int pthard = 1)
{
#ifdef __CINT__
  ...
  TFile *fin;
  fin = TFile::Open(fnamedata);
  fin->ls();
  TTree *tree = (TTree*) fin->Get("Preprocessed");
  tree->Show(0);
  ...
  n_weight_bins = 20;
  Double_t weight_bins[n_weight_bins] = (Double_t*) logbins(1e-5, 0, n_weight_bins);

  n_RL_bins = 20;
  Double_t RL_bins[n_RL_bins] = (Double_t*) logbins(1e-4, 0, n_RL_bins);

  n_pt_bins = 20;
  Double_t pt_bins[n_pt_bins] = (Double_t*) linbins(20, 40, n_pt_bins);


  //the raw correlation
  TH3D *h3_raw(0);
  h3_raw = new TH3D("r","raw", n_weight_bins,weight_bins, n_RL_bins,RL_bins, n_pt_bins,pt_bins);
  TH1D* h1_raw = new TH1D("h1_raw", "h1_raw", n_pt_bins, pt_bins);

  //detector measure level
  TH3D *h3_smeared(0);
  h3_smeared = new TH3D("smeared","smeared", n_weight_bins,weight_bins, n_RL_bins,RL_bins, n_pt_bins,pt_bins);
  TH1D* h1_smeared = new TH1D("h1_smeared", "h1_smeared", n_pt_bins, pt_bins);
  ...
  Double_t gen_energy_weight, gen_R_L, gen_jet_pt, obs_energy_weight, obs_R_L, obs_jet_pt;
  ...
  tree->SetBranchAddress("gen_energy_weight", &gen_energy_weight); 
  tree->SetBranchAddress("gen_R_L", &gen_R_L);
  tree->SetBranchAddress("gen_jet_pt", &gen_jet_pt); 
  tree->SetBranchAddress("obs_energy_weight", &obs_energy_weight); 
  tree->SetBranchAddress("obs_R_L", &obs_R_L);
  tree->SetBranchAddress("obs_jet_pt", &obs_jet_pt); 
  ...
  */
  ...
  for(int iEntry=0; iEntry< nEv; iEntry++){
   ...
   h3_raw->Fill(gen_energy_weight, gen_R_L, gen_jet_pt);
   h3_smeared->Fill(obs_energy_weight, obs_R_L, obs_jet_pt);
